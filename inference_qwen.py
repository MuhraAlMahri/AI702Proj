...
# 📌 创建输出目录
os.makedirs("output/qwen", exist_ok=True)
...
for shot_count in few_shot_counts:
    ...
    for i in tqdm(range(0, len(test_data), batch_size), desc=f"Processing {shot_count}-shot"):
        ...
        for idx, output in enumerate(batch_outputs):
            ...
            results.append({
                ...
                "model_output": response  # Qwen 生成的答案
            })

        # 每5条写入一次文件
        if (i + batch_size) % batch_size == 0:
            output_file = f"output/qwen/inference_results_qwen_{shot_count}shot.json"
    ...
    print(f"✅ {shot_count}-shot 版本完成，结果已保存到 output/qwen/inference_results_qwen_{shot_count}shot.json")
