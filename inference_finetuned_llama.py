...
# 加载LoRA适配器
model = PeftModel.from_pretrained(base_model, "./lora_llama3")
...
# 创建输出目录
os.makedirs("output/finetuned_llama", exist_ok=True)

# 定义不同数量的few-shot示例
few_shot_counts = [0, 2, 4, 6, 8, 10]
...
for shot_count in few_shot_counts:
    ...
    for i in tqdm(range(0, len(test_data), batch_size), desc=f"Processing {shot_count}-shot"):
        ...
        # 每处理完一个批次就写入文件
        output_file = f"output/finetuned_llama/inference_results_finetuned_{shot_count}shot.json"
